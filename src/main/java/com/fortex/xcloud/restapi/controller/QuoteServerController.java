...
import com.fortex.xcloud.restapi.model.QuoteServer;
...
@Slf4j
@RestController
@RequestMapping("/quoteServer")
public class QuoteServerController extends BaseController{
    @RequestMapping(method = RequestMethod.POST)
    public ResponseObj update(@RequestBody QuoteServer quoteServer) {
        ResponseObj responseObj = null;
        try {
            Config config = readObject();
            config.setQuoteServer(quoteServer);
            writeObject(config);
    ...
    public ResponseObj read(){
        ResponseObj responseObj = null;
        try {
            responseObj = new ResponseObj("QuoteServer",readObject().getQuoteServer(), ResponseContants.CODE_SUCCESS, ResponseContants.MESSAGE_SUCCESS);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            responseObj = new ResponseObj( ResponseContants.CODE_READ_FILE_ERROR, ResponseContants.MESSAGE_READ_FILE_ERROR);
        }
